...
class SiteController extends Controller
{
	...
	public function actionCategory($url)
	{
		...
		$q = new CDbCriteria;
		$q->addSearchCondition('tcategory.url', $url, true, 'OR', 'LIKE');
	...
	public function actionSubcategory($url)
	{
		...
		$q = new CDbCriteria;
		$q->addSearchCondition('typessubCategory.url', $url, true, 'OR', 'LIKE');
	...
	public function actionTypes($url)
	{
		...
		$q = new CDbCriteria;
		$q->addSearchCondition('types.url', $url, true, 'OR', 'LIKE');
	...
	public function actionBrands($url)
	{
		...
		$q = new CDbCriteria;
		$q->addSearchCondition('brands.url', $url, true, 'OR', 'LIKE');
	...
	public function updateData($product){
		...
		$cmd = 'java -jar C:/xampp/htdocs/dealerprice/crawler/Crawler.jar '.$product->id;
	...
	public function actionSearch($q, $cat)
	{
		if(isset($q) && isset($cat))
		{
			...
			$q = new CDbCriteria();
			$q->addSearchCondition('tcategory.name', $match, true, 'OR', 'LIKE');
	...
	{
		if(isset($_POST['email']) && !empty($_POST['email']) && isset($_POST['target']) && !empty($_POST['target']))
		{
			$reg = new AlertUsers;
