				<?php foreach($products as $prod){ 
                    ...
                    <div class="product_item keySpec" data-prodid="<?=$prod->id?>">
                        ...
                        <div class="description">
							<?=CHtml::link(CString::truncate($prod->name, 50, '...', false), array('site/products', 'url'=>$prod->url), array('class'=>'productTitle'))?>
                            <div class="clearfix product_info">
								<?php if($prod->productPrice->discount > 0 && $prod->productPrice->mrp > 0){ ?>
                                <p class="product_price"><span class="discount"><span class="discountRs">Rs. <?=number_format($prod->productPrice->mrp)?></span><span class="discountRate">(<?=$prod->productPrice->discount?>% off)</span></span></p>
								<?php } ?>
