...
class UsersController extends Controller
{
		...
		if(isset($_POST['ForgetForm']))
		{
			$model->attributes=$_POST['ForgetForm'];
                        ...
                        if($validate)
                        {
                            ...
                            if(isset($user))
                            {
                               CMailSend::forget($user);
                               echo CJSON::encode(array('status'=>'success', 'msg'=> 'We have been sent password to your email.'));
                               Yii::app()->end();
                            }
                            else
                            {
                                echo CJSON::encode(array('status'=>'failed', 'msg'=>'User not found with this email.'));
                                Yii::app()->end();
                            }
                        }
                        else
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Users']))
		{
			...
			if($model->save())
			{
				CMailSend::signup($model);
				$login=new LoginForm;
				$login->email=$model->email;
				$login->password=$model->password;
				$login->login();
				echo CJSON::encode(array('status'=>'success', 'redirect'=>  $this->createAbsoluteUrl('/site/index')));
			   Yii::app()->end();
			}
			else
			{
