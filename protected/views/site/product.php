            ...
            <div id="pageHeader">
                <div id="innerPageHeader">
                    ...
                    <div id="breadcrumb" style=' width: 620px;'>
						<?php if(isset($product->brands) && isset($product->brands->brandssubCategory->category)){ ?>
						...
						<?php } else  if(isset($product->types) && isset($product->types->typessubCategory->category)) { ?>
						<div id="breadcrumbItm">
						<span id="bcPnt"><?=CHtml::link($product->types->typessubCategory->category->name, array('site/category', 'url'=>$product->types->typessubCategory->category->url)); ?></span> <span class="nextItm">&gt;</span> <span class="bcItm"><?=CHtml::link($product->types->typessubCategory->name, array('site/subcategory', 'url'=>$product->types->typessubCategory->url)); ?></span><span class="nextItm">&gt;</span><span class="bcItm"><?=CHtml::link($product->types->name, array('site/types', 'url'=>$product->types->url)); ?></span><span class="nextItm">&gt;</span><span class="bcItm lastItm"><?=$product->name?></span><span id="storeCount"> (<?=count($product->productDetails)?> Stores)</span>
                        </div>
						<?php } ?>
                    </div>
                    <!-- End of Breadcrumb -->
                        ...
                        <div id="itmContentTwoLeft">
							...
							<section class="section_offset">
								<?php
									...
									$q->order='RAND()';
									$q->group = 'products.id';
									if(isset($product->brands) && isset($product->types))
									$q->condition = "products.id <>".$product->id.' AND (brands.id ='.$product->brands->id. ' OR types.id = '.$product->types->id.')';
									else if(isset($product->brands))
									$q->condition = "products.id <>".$product->id.' AND brands.id ='.$product->brands->id;
									else if(isset($product->types))
									$q->condition = "products.id <>".$product->id.' AND types.id = '.$product->types->id;
									else
									$q->condition = "products.id <>".$product->id;	
								...
								?>
										...
										<div class="product_item">
											...
											<div class="description">
												<?=CHtml::link(CString::truncate($tprod->name, 50, '...', false), array('site/products', 'url'=>$tprod->url), array('class'=>'productTitle'))?>
									...
									<?php } ?>
										...
										<li>
											...
											<div class="wrapper">
												...
												<?=CHtml::link(CString::truncate($tprod->name, 50, '...', false), array('site/products', 'url'=>$tprod->url), array('class'=>'product_title'))?>
							...
							<section class="section_offset">
								...
								?>
									<li>
										...
										<div class="wrapper">
											<?=CHtml::link(CString::truncate($prod->name, 50, '...', false), array('site/products', 'url'=>$prod->url), array('class'=>'product_title'))?>
